...
export default function PaymentScreen() {
  ...
  const openFinal = async () => {
    setLoading(true);
    try {
      const { data } = await api.post('/api/payments/final', { jobId });
      payKindRef.current = 'final';
      setWv({
        visible: true,
        payUrl: data.payUrl,
        fields: data.fields,
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.step}>
          <View style={styles.stepRow}>
            ...
            <Text style={styles.stepText}>Step 1 — 50% deposit</Text>
          </View>
          {!job?.depositPaid ? (
            <GoldButton title="Pay Deposit" onPress={openDeposit} loading={loading} />
          ) : (
            <Text style={styles.paid}>Paid — thank you</Text>
          )}
        </View>
        ...
        <View style={styles.step}>
          <View style={styles.stepRow}>
            <View style={styles.stepDot} />
            <Text style={styles.stepText}>Step 2 — Remaining 50%</Text>
          </View>
          {job?.depositPaid && !job?.finalPaid ? (
            <GoldButton title="Pay Final Amount" onPress={openFinal} loading={loading} />
          ) : null}
          {job?.finalPaid ? (
            <Text style={styles.paid}>All payments received</Text>
          ) : null}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  h1: { color: COLORS.text, fontSize: 24, fontWeight: '700', marginBottom: 16 },
  card: {
